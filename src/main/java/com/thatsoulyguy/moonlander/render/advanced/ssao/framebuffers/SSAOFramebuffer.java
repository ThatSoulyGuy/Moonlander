...
public class SSAOFramebuffer extends Framebuffer
{
    ...
    public void generate()
    {
        ...
        GL41.glFramebufferTexture2D(GL41.GL_FRAMEBUFFER, GL41.GL_COLOR_ATTACHMENT0, GL41.GL_TEXTURE_2D, ssaoColor, 0);

        int status = GL41.glCheckFramebufferStatus(GL41.GL_FRAMEBUFFER);
        if (status != GL41.GL_FRAMEBUFFER_COMPLETE)
            System.err.println("SSAOBuffer not complete, status: " + status);
    ...
    {
        return Collections.singletonMap("ssaoColor", ssaoColor);
    }
    ...
    public void uninitialize()
    {
        ...
        GL41.glDeleteTextures(ssaoColor);
    }
}